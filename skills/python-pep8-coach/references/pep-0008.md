...
in the C implementation of Python (PEP 7).

This document and PEP 257 (Docstring
...
Python code. As PEP 20 says, "Readability
counts".
...
described in the
[policy section of PEP 3131](https://peps.python.org/pep-3131/#policy-specification).
...
CapWords preferring short names: `T`,
...
- When implementing ordering operations with rich comparisons, it is
  ...
  PEP 207 indicates that reflexivity rules
...
- Derive exceptions from `Exception` rather than `BaseException`. Direct
  ...
  *catching* the exceptions is likely to need, rather than the locations
  ...
  problem occurred" (see PEP 3151 for an
  example of this lesson being learned for the builtin exception
  hierarchy)
...
- Use of the flow control statements `return`/`break`/`continue` within
  the finally suite of a `try...finally`, where the flow control
...
With the acceptance of PEP 484, the style
rules for function annotations have changed.

- Function annotations should use PEP 484
...
- However, outside the stdlib, experiments within the rules of
  PEP 484 are now encouraged. For example,
  marking up a large third party library or application with
  PEP 484 style type annotations, reviewing
  how easy it was to add those annotations, and observing whether their
...
- Users who don\'t want to use type checkers are free to ignore them.
  ...
  .pyi files that are read by the type checker in preference of the
  corresponding .py files. Stub files can be distributed with a library,
  or separately (with the library author\'s permission) through the
  typeshed repo[^5].

## Variable Annotations

PEP 526 introduced variable annotations. The
...
- Although PEP 526 is accepted for
  Python 3.6, the variable annotation syntax is the preferred syntax for
  stub files on all versions of Python (see PEP 484 for details).
